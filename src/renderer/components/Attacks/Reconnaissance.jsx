...
function Reconnaissance({ attacker, attacks, isLoading, machines, setMachines, handleRefresh }) {
  ...
  const onPortPortsValueChange = (e) => {
    ...
    if (!ok) {
      // FIX: NON disabilitare il flag mentre si digita, altrimenti perde il focus!
      // if (portPortsEnabled) setPortPortsEnabled(false);

      // Mostra warning
      setPortPortsWarning("Invalid ports format (e.g. 22,80,100-200)");

      // non ricostruire gli args finché non è valido
      return;
    }

    // se arriviamo qui => spec valida
    // Rimuovi warning se c'era
    setPortPortsWarning(null);

    // se per caso era disabilitato (ma qui lo vogliamo attivo), attiviasmolo

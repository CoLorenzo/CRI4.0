		...
		if (machine.name && machine.name != "" && machine.type == "other" && machine.other.image) {
			...
			if (Array.isArray(machine.other.envDefs)) {
				for (const envDef of machine.other.envDefs) {
					if (envDef.type === "file") {
						...
						lab.file["lab.conf"] += `${machine.name}[env]="${envDef.key}=${envDef.value}"\n`;
					}
				}
			}
			if (Array.isArray(machine.other.dockerFlags) && machine.other.dockerFlags.length > 0) {
				const opts = machine.other.dockerFlags
					...
					.join(" ");
				if (opts) lab.file["lab.conf"] += `${machine.name}[docker_options]="${opts}"\n`;
			}
			...
			const userScript = (machine.scripts && typeof machine.scripts.startup === 'string'
				...
				: '') || (machine.interfaces && typeof machine.interfaces.free === 'string'
					? machine.interfaces.free
					: '');
			if (machine.customTemplateId && userScript && userScript.trim()) {
				lab.file[machine.name + '.startup'] = userScript;
			} else {
...
function makeIndustrialDevices(netkit, lab) {
	for (let machine of netkit) {
		...
		if (machine.name && machine.name != "" && machine.type === "scada") {
			let monitoredMachines = [];
			if (machine.industrial && machine.industrial.monitored_machines && Array.isArray(machine.industrial.monitored_machines)) {
				for (const monitoredId of machine.industrial.monitored_machines) {
					...
					if (targetMachine) {
						...
						if (!targetIp && targetMachine.interfaces?.if?.[0]?.ip) {
			...
			lab.file[machine.name + ".startup"] += `cat << 'MONITORED_EOF' >> /root/.bashrc\nexport MONITORED_MACHINES='${safeJson}'\nMONITORED_EOF\n`;
		}
	}
}
